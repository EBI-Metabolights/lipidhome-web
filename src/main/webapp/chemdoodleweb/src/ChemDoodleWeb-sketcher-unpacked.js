...
(function(math, monitor, actions, states, structures, SYMBOLS, inArray, m) {
	...
	states._State.prototype.getOptimumAngle = function(a) {
		...
		if (angles.length == 0) {
			...
			angle = math.angleBetweenLargest(angles);
		}
		return angle;
	};
...
(function(math, monitor, actions, states, structures, inArray, m) {
	...
	states.NewRingState.prototype.getOptimalRing = function(b, numSides) {
		...
		for ( var i = 1, ii = ring1.length; i < ii; i++) {
			for ( var j = 0, jj = this.sketcher.molecule.atoms.length; j < jj; j++) {
				dist1 += m.min(1E8, 1 / m.pow(this.sketcher.molecule.atoms[j].distance(ring1[i]), 2));
				dist2 += m.min(1E8, 1 / m.pow(this.sketcher.molecule.atoms[j].distance(ring2[i]), 2));
			}
		}
		if (dist1 < dist2) {
			return ring1;
		...
		if (this.sketcher.hovering instanceof structures.Atom) {
			...
			if (e.p.distance(this.sketcher.hovering) < 15) {
				...
				if (angles.length == 0) {
					...
					a = math.angleBetweenLargest(angles);
				}
...
//  $Revision: 3211 $
//  $Author: kevin $
//  $LastChangedDate: 2011-04-28 17:29:52 -0400 (Thu, 28 Apr 2011) $
//
...
(function(c, actions, gui, desktop, q) {
	...
	gui.DialogManager = function(sketcher) {
		if (sketcher.useServices) {
			this.saveDialog = new desktop.SaveFileDialog('save_dialog', sketcher);
		} else {
			this.saveDialog = new desktop.Dialog('save_dialog', 'Save Molecule');
		...
		this.loadDialog = new desktop.Dialog('load_dialog', 'Load Molecule');
		...
		this.searchDialog = new desktop.MolGrabberDialog('search_dialog');
		...
		this.periodicTableDialog = new desktop.PeriodicTableDialog('periodicTable_dialog');
		...
		this.calculateDialog = new desktop.Dialog('calculate_dialog', 'Calculations');
...
//  $Revision: 3361 $
//  $Author: kevin $
//  $LastChangedDate: 2011-08-18 18:32:22 -0400 (Thu, 18 Aug 2011) $
//
(function(desktop, q, document) {
...
(function(c, iChemLabs, io, actions, gui, desktop, structures, q, document) {
	...
	gui.ToolbarManager = function(sketcher) {
		...
		// open
		this.buttonOpen = new desktop.Button('button_open', sketcher.iconPath, 'open20', 'Open', function() {
			sketcher.dialogManager.loadDialog.getTextArea().val('');
			sketcher.dialogManager.loadDialog.getElement().dialog('open');
		});
		// save
		this.buttonSave = new desktop.Button('button_save', sketcher.iconPath, 'save20', 'Save', function() {
			if(sketcher.useServices){
		...
		// search
		this.buttonSearch = new desktop.Button('button_search', sketcher.iconPath, 'search20', 'Search', function() {
			sketcher.dialogManager.searchDialog.getElement().dialog('open');
		});
		// calculate
		this.buttonCalculate = new desktop.Button('button_calculate', sketcher.iconPath, 'calculate20', 'Calculate', function() {
			iChemLabs.calculate(sketcher.molecule, ['mf', 'mw', 'miw'], function(content){
				var sb = [];
				function addDatum(title, value){
					sb.push(title);
					sb.push(': ');
					for(var i = title.length+2; i<25; i++){
						sb.push(' ');
					}
					sb.push(value);
					sb.push('\n');
				...
				addDatum('Molecular Mass', content.mw);
				addDatum('Monoisotopic Mass', content.miw);
		...
		// move
		this.buttonMove = new desktop.Button('button_move', sketcher.iconPath, 'move20', 'Move', function() {
			sketcher.stateManager.currentState = sketcher.stateManager.STATE_MOVE;
		});
		this.buttonMove.toggle = true;
		// erase
		this.buttonErase = new desktop.Button('button_erase', sketcher.iconPath, 'erase20', 'Erase', function() {
			sketcher.stateManager.currentState = sketcher.stateManager.STATE_ERASE;
		});
		this.buttonErase.toggle = true;

		// clear
		this.buttonClear = new desktop.Button('button_clear', sketcher.iconPath, 'clear20', 'Clear', function() {
			var clear = true;
		...
		// clean
		this.buttonClean = new desktop.Button('button_clean', sketcher.iconPath, 'optimize20', 'Clean', function() {
	...
	gui.ToolbarManager.prototype.write = function() {
		...
		sb.push(this.chargeSet.getSource());
		sb.push('<br>');
	...
	gui.ToolbarManager.prototype.makeScaleSet = function(self) {
		this.scaleSet = new desktop.ButtonSet('buttons_scale');
		this.scaleSet.toggle = false;
		this.buttonScalePlus = new desktop.Button('button_scale_plus', self.sketcher.iconPath, 'zoomIn20', 'Increase Scale', function() {
		...
		this.buttonScaleMinus = new desktop.Button('button_scale_minus', self.sketcher.iconPath, 'zoomOut20', 'Decrease Scale', function() {
			self.sketcher.specs.scale /= 1.5;
			self.sketcher.checkScale();
			self.sketcher.repaint();
		});
	...
	gui.ToolbarManager.prototype.makeHistorySet = function(self) {
		this.historySet = new desktop.ButtonSet('buttons_history');
		this.historySet.toggle = false;
		this.buttonUndo = new desktop.Button('button_undo', self.sketcher.iconPath, 'undo20', 'Undo', function() {
			self.sketcher.historyManager.undo();
		});
		this.historySet.buttons.push(this.buttonUndo);
		this.buttonRedo = new desktop.Button('button_redo', self.sketcher.iconPath, 'redo20', 'Redo', function() {
	...
	gui.ToolbarManager.prototype.makeLabelSet = function(self) {
		this.labelSet = new desktop.ButtonSet('buttons_label');
		this.buttonLabel = new desktop.DummyButton('button_label', self.sketcher.iconPath, 'Carbon', 'Set Label');
		this.labelSet.buttons.push(this.buttonLabel);
		this.labelSet.addDropDown('More Labels', self.sketcher.iconPath);
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_h', self.sketcher.iconPath, 'Hydrogen', 'Hydrogen', function() {
		...
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_c', self.sketcher.iconPath, 'Carbon', 'Carbon', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
		...
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_n', self.sketcher.iconPath, 'Nitrogen', 'Nitrogen', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'N';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_o', self.sketcher.iconPath, 'Oxygen', 'Oxygen', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'O';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_f', self.sketcher.iconPath, 'Fluorine', 'Fluorine', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'F';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_cl', self.sketcher.iconPath, 'Chlorine', 'Chlorine', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'Cl';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_br', self.sketcher.iconPath, 'Bromine', 'Bromine', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'Br';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_i', self.sketcher.iconPath, 'Iodine', 'Iodine', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'I';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_p', self.sketcher.iconPath, 'Phosphorus', 'Phosphorus', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'P';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_s', self.sketcher.iconPath, 'Sulfur', 'Sulfur', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_LABEL;
			self.sketcher.stateManager.STATE_LABEL.label = 'S';
		}));
		this.labelSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_label_pt', self.sketcher.iconPath, 'periodicTable20', 'Choose Symbol', function() {
	...
	gui.ToolbarManager.prototype.makeBondSet = function(self) {
		this.bondSet = new desktop.ButtonSet('buttons_bond');
		this.buttonSingle = new desktop.Button('button_bond_single', self.sketcher.iconPath, 'SingleBond', 'Single Bond', function() {
		...
		this.buttonRecessed = new desktop.Button('button_bond_recessed', self.sketcher.iconPath, 'RecessedBond', 'Recessed Bond', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_BOND;
			self.sketcher.stateManager.STATE_NEW_BOND.bondOrder = 1;
		...
		this.buttonProtruding = new desktop.Button('button_bond_protruding', self.sketcher.iconPath, 'ProtrudingBond', 'Protruding Bond', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_BOND;
			self.sketcher.stateManager.STATE_NEW_BOND.bondOrder = 1;
		...
		this.buttonDouble = new desktop.Button('button_bond_double', self.sketcher.iconPath, 'DoubleBond', 'Double Bond', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_BOND;
			self.sketcher.stateManager.STATE_NEW_BOND.bondOrder = 2;
			self.sketcher.stateManager.STATE_NEW_BOND.stereo = structures.Bond.STEREO_NONE;
		});
		this.bondSet.buttons.push(this.buttonDouble);
		this.buttonBond = new desktop.DummyButton('button_bond', self.sketcher.iconPath, 'TripleBond', 'Other Bond');
		this.bondSet.buttons.push(this.buttonBond);
		this.bondSet.addDropDown('More Bonds', self.sketcher.iconPath);
		this.bondSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_bond_triple', self.sketcher.iconPath, 'TripleBond', 'Triple Bond', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_BOND;
		...
		this.bondSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_bond_ambiguous_double', self.sketcher.iconPath, 'AmbiguousDoubleBond', 'Ambiguous Double Bond', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_BOND;
			self.sketcher.stateManager.STATE_NEW_BOND.bondOrder = 2;
	...
	gui.ToolbarManager.prototype.makeRingSet = function(self) {
		this.ringSet = new desktop.ButtonSet('buttons_ring');
		this.buttonCyclohexane = new desktop.Button('button_ring_cyclohexane', self.sketcher.iconPath, 'Cyclohexane', 'Cyclohexane Ring', function() {
		...
		this.buttonBenzene = new desktop.Button('button_ring_benzene', self.sketcher.iconPath, 'Benzene', 'Benzene Ring', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_RING;
			self.sketcher.stateManager.STATE_NEW_RING.numSides = 6;
			self.sketcher.stateManager.STATE_NEW_RING.unsaturated = true;
		});
		this.ringSet.buttons.push(this.buttonBenzene);
		this.buttonBond = new desktop.DummyButton('button_ring', self.sketcher.iconPath, 'Cyclopentane', 'Other Ring');
		this.ringSet.buttons.push(this.buttonBond);
		this.ringSet.addDropDown('More Rings', self.sketcher.iconPath);
		this.ringSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_ring_cycloproane', self.sketcher.iconPath, 'Cyclopropane', 'Cyclopropane Ring', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_RING;
			self.sketcher.stateManager.STATE_NEW_RING.numSides = 3;
			self.sketcher.stateManager.STATE_NEW_RING.unsaturated = false;
		}));
		this.ringSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_ring_cyclobutane', self.sketcher.iconPath, 'Cyclobutane', 'Cyclobutane Ring', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_RING;
			self.sketcher.stateManager.STATE_NEW_RING.numSides = 4;
			self.sketcher.stateManager.STATE_NEW_RING.unsaturated = false;
		}));
		this.ringSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_ring_cyclopentane', self.sketcher.iconPath, 'Cyclopentane', 'Cyclopentane Ring', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_RING;
		...
		this.ringSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_ring_cycloheptane', self.sketcher.iconPath, 'Cycloheptane', 'Cycloheptane Ring', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_RING;
			self.sketcher.stateManager.STATE_NEW_RING.numSides = 7;
			self.sketcher.stateManager.STATE_NEW_RING.unsaturated = false;
		}));
		this.ringSet.dropDown.buttonSet.buttons.push(new desktop.Button('button_ring_cyclooctane', self.sketcher.iconPath, 'Cyclooctane', 'Cyclooctane Ring', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_NEW_RING;
			self.sketcher.stateManager.STATE_NEW_RING.numSides = 8;
			self.sketcher.stateManager.STATE_NEW_RING.unsaturated = false;
		}));
	};
	gui.ToolbarManager.prototype.makeChargeSet = function(self) {
		this.chargeSet = new desktop.ButtonSet('buttons_charge');
		this.buttonPlus = new desktop.Button('button_charge_increment', self.sketcher.iconPath, 'IncreaseCharge', 'Increase Charge', function() {
		...
		this.chargeSet.buttons.push(this.buttonPlus);
		this.buttonMinus = new desktop.Button('button_charge_decrement', self.sketcher.iconPath, 'DecreaseCharge', 'Decrease Charge', function() {
			self.sketcher.stateManager.currentState = self.sketcher.stateManager.STATE_CHARGE;
			self.sketcher.stateManager.STATE_CHARGE.delta = -1;
		});
		this.chargeSet.buttons.push(this.buttonMinus);
	};
...
(function(c, extensions, sketcherPack, structures, q, m, window) {
	...
	c.SketcherCanvas.prototype.drawChildExtras = function(ctx) {
		...
		// zoom in for mobile devices
		if (false && this.isMobile && this.lastPoint != null) {
			ctx.save();
			ctx.strokeStyle = 'black';
			ctx.lineWidth = 1;
			var dim = this.width / 4;
			...
			ctx.clip();
			ctx.scale(2, 2);
			ctx.translate(-this.lastPoint.x, -this.lastPoint.y);
			ctx.translate(dim / 4, dim / 4);
